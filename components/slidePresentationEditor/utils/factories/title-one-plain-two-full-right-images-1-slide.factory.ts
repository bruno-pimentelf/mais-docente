...
/**
 * Factory for creating title one plain two full right images 1 slides
 ...
 * - Both images have aspect ratio of 1.79 and same dimensions
 * Follows Single Responsibility Principle - only creates title one plain two full right images 1 slides
 */
export class TitleOnePlainTwoFullRightImages1SlideFactory
  implements ITitleOnePlainTwoFullRightImagesSlideFactory
{
  ...
  create(
    ...
    // Both images have fixed height of 524px with 32px gap between them
    const imageAspectRatio = SLIDE_IMAGE_RATIOS._1_79;
    const imageHeight = 524; // Fixed height
    const imageWidth = imageHeight * imageAspectRatio; // Calculate width from aspect ratio
    const rightSideX = slideWidth - imageWidth; // Images touch the right edge
    ...
    const imageElement2: SlideImage = {
      ...
      src: image_url_2,
      x: rightSideX,
      y: slideHeight - imageHeight, // Touches bottom
      width: imageWidth,
      height: imageHeight,
    ...
    return {
      ...
      slideType: SlideThemeType.TITLE_ONE_PLAIN_TWO_FULL_RIGHT_IMAGES_1,
      themeSettings: {

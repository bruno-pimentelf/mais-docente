...
export class TitleThreeRowsOneLeftImage5SlideFactory
  implements ITitleThreeRowsOneLeftImageSlideFactory
{
   ...
   * Uses fixed aspect ratio of 1.44 and vertically centers the image in the slide
   */
  private calculateAspectRatioDimensions(
    maxWidth: number,
    maxHeight: number
  ): { width: number; height: number; x: number; y: number } {
    const aspectRatio = SLIDE_IMAGE_RATIOS._1_44;
    ...
    // Center the image vertically in the entire slide
  ...
  async create(
    ...
    return {
      ...
      slideType: SlideThemeType.TITLE_THREE_ROWS_ONE_LEFT_IMAGE_5,
      themeSettings: {

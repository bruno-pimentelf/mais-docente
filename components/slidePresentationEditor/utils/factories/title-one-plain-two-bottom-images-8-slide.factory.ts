...
export class TitleOnePlainTwoBottomImages8SlideFactory implements ITitleOnePlainTwoBottomImagesSlideFactory {
  ...
  create(
    ...
    // Calculate image widths based on aspect ratios and height
    const gap = 62;

    const firstImageWidth = imageHeight * SLIDE_IMAGE_RATIOS._1_01;
    const secondImageWidth = imageHeight * SLIDE_IMAGE_RATIOS._0_74;

    // Calculate total width needed and center the images horizontally
    const totalImagesWidth = firstImageWidth + gap + secondImageWidth;
    const startX = (slideWidth - totalImagesWidth) / 2;

    const firstImageX = startX;
    const secondImageX = startX + firstImageWidth + gap;
    ...
    return {
      ...
      slideType: SlideThemeType.TITLE_ONE_PLAIN_TWO_BOTTOM_IMAGES_8,
      themeSettings: {

...
export class TitleOnePlainThreeBottomImages3SlideFactory implements ITitleOnePlainThreeBottomImagesSlideFactory {
  ...
  create(
    ...
    // Three images in a single row with specific widths
    const gap = 36;
    const imagesAreaX = textX;
    const baseY = contentTextElement.y + contentTextElement.height + 60;
    const imageHeight = slideHeight - SLIDE_MARGIN - baseY; // Fill available space up to margin
    const image1Width = 405;
    const image2Width = 750;
    const image3Width = 405; // Same as first image
    ...
    const image3Elements = image_url_3
      ? createImageElement({
          image: {
            src: image_url_3,
            x: imagesAreaX + image1Width + gap + image2Width + gap,
            y: baseY,
            width: image3Width,
            height: imageHeight,
            borderRadius: 0,
    ...
    return {
      ...
      slideType: SlideThemeType.TITLE_ONE_PLAIN_THREE_BOTTOM_IMAGES_3,
      themeSettings: {

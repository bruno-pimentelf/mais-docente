...
export class TitleOnePlainThreeBottomImages5SlideFactory implements ITitleOnePlainThreeBottomImagesSlideFactory {
  ...
  create(
    ...
    const image3Width = Math.floor(imageHeight * SLIDE_IMAGE_RATIOS._1_01);

    // Calculate total width of all images plus gaps
    const totalImagesWidth =
      image1Width + gap + image2Width + gap + image3Width;

    // Center the images row horizontally in the slide
    const imagesRowStartX = Math.floor((slideWidth - totalImagesWidth) / 2);

    // Calculate x positions for each image
    const img1X = imagesRowStartX;
    const img2X = img1X + image1Width + gap;
    const img3X = img2X + image2Width + gap;
    ...
    return {
      ...
      slideType: SlideThemeType.TITLE_ONE_PLAIN_THREE_BOTTOM_IMAGES_5,
      themeSettings: {

...
export class TitleThreeColumnThreeTopImages2SlideFactory
  implements ITitleThreeColumnThreeTopImagesSlideFactory
{
  ...
  create(
    ...
    const cardWidth = Math.floor((cardsAreaWidth - cardGap * 2) / 3); // Two gaps between three cards
    ...
    const createCard = (
      ...
      const cardX = titleX + cardIndex * (cardWidth + cardGap);
      const cardY = cardsStartY;
      const textSpacing = 20;
      const imageSize = 525; // Square images of 525px
      ...
      if (cardImage) {
        const cardImageElement: SlideImage = {
          ...
          src: cardImage,
          x: cardX + (cardWidth - imageSize) / 2, // Center the image horizontally
          y: cardY,
          width: imageSize,
          height: imageSize,
      ...
      const cardTextElement: SlideText = {
        ...
        text: `<span style="overflow-wrap: break-word; color: ${this.colors.paragraphColor};">${processMarkdownFormatting(cardText)}</span>`,
        x: cardX,
        y: cardY + imageSize + textSpacing + cardTitleHeight + textSpacing,
        width: cardWidth,
    ...
    return {
      ...
      slideType: SlideThemeType.TITLE_THREE_COLUMN_THREE_TOP_IMAGES_2,
      themeSettings: {

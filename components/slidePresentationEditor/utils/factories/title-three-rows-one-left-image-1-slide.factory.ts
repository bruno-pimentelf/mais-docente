...
export class TitleThreeRowsOneLeftImage1SlideFactory
  ...
  async create(
    ...
    const sectionHeights = sectionData.map(({ title, text }) => {
      const titleHeight = getTextHeight({
        text: `<span style="overflow-wrap: break-word; color: ${this.colors.titleColor}; font-weight: bold;">${title}</span>`,
        fontSize: 40,
        fontFamily: 'Quicksand',
        lineHeight: 1.2,
        width: rightAreaWidth,
      ...
      const textHeight = getTextHeight({
        text: `<span style="overflow-wrap: break-word; color: ${this.colors.paragraphColor};">${text}</span>`,
        fontSize: 28,
        fontFamily: 'Quicksand',
        lineHeight: 1.4,
        width: rightAreaWidth,
    ...
    return {
      ...
      slideType: SlideThemeType.TITLE_THREE_ROWS_ONE_LEFT_IMAGE_1,
      themeSettings: {
